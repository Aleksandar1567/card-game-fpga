...
-- Description  : This file contains random numbers generator based on Linear-feedback shift 

-- register which gives random numbers in range [0, 24] which are used to define position in initial color 

-- array to create array of random colors
...
-- 18/12/2022       1                   Aleksandar, Aleksa      Created
...
entity random is
    port (
        ...
        was_previous : out std_logic; -- indikator da je došao zahtev sa ulaza previous_card
        was_next : out std_logic; -- indikator da je došao zahtev sa ulaza next_card
        was_select : out std_logic -- indikator da je došao zahtev sa ulaza select_card
        );
end random;
...
begin
    ...
    RememberProcess : process(clk,reset) is
    begin
		  if reset = '1' then
			was_previous <='0';
            was_next<='0';
    ...
    WriteProcess : process(clk,reset) is
    begin
		if reset ='1' then
		output <= (others =>'0');
      elsif (rising_edge(clk)) then 
